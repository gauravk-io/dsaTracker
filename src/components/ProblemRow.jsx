...
export default function ProblemRow({ problem, isCompleted, onToggle }) {
  ...
  return (
    <div className={`problem-row ${isCompleted ? 'completed' : ''}`} style={{ 
        display: 'flex', 
        alignItems: 'center', 
        padding: '1rem 1.5rem', 
      ...
      <label className="checkbox-container" style={{ marginRight: '1rem', cursor: 'pointer', position: 'relative', display: 'flex' }}>
        <input 
          type="checkbox" 
          checked={isCompleted} 
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {problem.url ? (
          <a href={problem.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', color: 'inherit', display: 'flex', alignItems: 'center', gap: '0.5rem' }} className="problem-link">
            <span style={{ 
                fontWeight: '500', 
                fontSize: '1rem', 
